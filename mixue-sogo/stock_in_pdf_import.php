...
// If also_jakel is set, the same import is mirrored into aero_foods_finance.
...
try {
    ...
    // --- Cross-DB sync to aero_foods_finance if requested ---
    $jakelResult = null;
    if (!empty($data['also_jakel'])) {
        $jakelPdo = new PDO("pgsql:host=$host;port=$port;dbname=aero_foods_finance", $user, $password);
        $jakelPdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $jakelPdo->beginTransaction();
        [$jakelInserted, $jakelSkipped, $jakelMatAdded, $jakelPriceUpdated] = runImport($jakelPdo, $data, $orderNumber, $monthDate, $snapshotDate, $year, $month, $day, $importUser);
        $jakelPdo->commit();
        $jakelResult = [
            'inserted' => $jakelInserted,
            'skipped_duplicates' => $jakelSkipped,
            'materials_added' => $jakelMatAdded,
            'prices_updated' => $jakelPriceUpdated,
        ];
    }
    ...
    if ($jakelResult !== null) {
        $response['jakel_sync'] = $jakelResult;
    }
    echo json_encode($response);

} catch (PDOException $e) {
    if (isset($pdo) && $pdo->inTransaction()) { $pdo->rollBack(); }
    if (isset($jakelPdo) && $jakelPdo->inTransaction()) { $jakelPdo->rollBack(); }
