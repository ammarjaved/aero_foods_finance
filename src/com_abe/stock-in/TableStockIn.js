...
function TableStockIn() {
  ...
  const handleSaveEdit = async (itemId) => {
    try {
      const response = await fetch('http://121.121.232.54:88/abe-yus/update_stockin_trans.php', {
        method: 'POST',
        headers: {
  ...
  const handleConfirmDelete = async (itemId,month_date) => {
    try {
      const response = await fetch('http://121.121.232.54:88/abe-yus/del_stock.php', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
