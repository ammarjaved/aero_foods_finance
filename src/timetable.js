...
function Timetable({ onRowClick }) {
  ...
  const handleRecordUpdate = (event) => {
    ...
    setFilteredData(prev => prev.map(r => r.id === updatedRecord.id ? updatedRecord : r));
  };
  ...
  const applyFilters = () => {
    ...
    if (nameFilter) {
      filtered = filtered.filter(record => 
        record.name && record.name.toLowerCase().includes(nameFilter.toLowerCase())
      );
    }
  ...
  const uniqueNames = Array.from(new Set(data.map(record => record.name))).filter(Boolean);
                  ...
                  currentRecords.map((record) => (
                    <tr key={record.id}>
                          ...
                          {editRowId === record.id && col.key !== 'id' ? (
                            col.key === 'name' ? (
                              ...
                              <input
                                ...
                                value={editFormData[col.key]?.slice(0, 16) || ''}
                                onChange={(e) => handleEditChange(e, col.key)}
                              />
                            ) : col.key === 'image_start_time' || col.key === 'image_end_time' ? (
                              <div className="d-flex flex-column gap-1">
                                <input
                                  type="file"
                                  accept="image/*"
                                  className="form-control form-control-sm"
                                  onChange={(e) => handleImageUpload(e, col.key)}
                                />
                               {editFormData[col.key] && (
                                  <div className="mt-1">
                                    <img 
                                      src={
                                        typeof editFormData[col.key] === 'string' && !editFormData[col.key].startsWith('data:') 
                                          ? `http://121.121.232.54:88/aero-foods/${editFormData[col.key]}`
                                          : typeof editFormData[col.key] === 'string' && editFormData[col.key].startsWith('data:')
                                            ? editFormData[col.key] // It's already a data URL
                                            : URL.createObjectURL(editFormData[col.key]) // It's a File object
                                      }
                                      alt={`${col.key} preview`}
                              ...
                              <input
                                ...
                                onChange={(e) => handleEditChange(e, col.key)}
                              />
                            ) : col.key === 'total_hr' || col.key === 'ot' || col.key === 'meal' ? (
                              <input
                                type="text"
                                className="form-control form-control-sm"
                              ...
                              <div className="d-flex flex-column align-items-center gap-1">
                                {record[col.key] ? (
                                  <>
                                    ...
                                    <img 
                                      src={'http://121.121.232.54:88/aero-foods/'+record[col.key]} 
                                      alt={`${col.key}`}
                                      className="img-thumbnail"
                                      style={{ maxHeight: '40px', cursor: 'pointer' }}
                                      onClick={() => handleImageView('http://121.121.232.54:88/aero-foods/'+record[col.key])}
                                    />
                                  </>
                                ) : (
                            ...
                            col.key.includes('hr') || col.key === 'break_time' || col.key === 'ot'
                      ...
                      <td>
                        {editRowId === record.id ? (
                          <>
                            <button className="btn btn-sm btn-success me-1" onClick={() => handleSave(record.id)}>Save</button>
                            <button className="btn btn-sm btn-secondary" onClick={handleCancel}>Cancel</button>
                          </>
                        ) : (
                          <>
                            <button className="btn btn-sm btn-warning me-1" onClick={() => handleEditClick(record)}>Edit</button>
