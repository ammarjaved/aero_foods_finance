...
function TableMonthSummary7() {
  ...
  const fetchMonthData = async (month, year) => {
    try {
      const response = await fetch(
        `http://121.121.232.54:88/mixue-sogo/mon-sum-mixiue7.php?month=${month}&year=${year}`
      );
      ...
      const processedData = fetchedData.monthly_data
        .map((item) => ({
          ...item,
          total_sales:
            parseFloat(item.total_sales) === 0 ? null : parseFloat(item.total_sales),
          total_actual:
            parseFloat(item.total_actual) === 0 ? null : parseFloat(item.total_actual),
          total_variance:
            parseFloat(item.total_variance) === 0 ? null : parseFloat(item.total_variance),
          total_expense:
            parseFloat(item.total_expense) === 0 ? null : parseFloat(item.total_expense),
      ...
      setAllMonthsData((prev) => ({ ...prev, [cacheKey]: processedData }));
      ...
      if (allMonthsData[monthYear]) {
        allMonthsData[monthYear].forEach((item) => {
          combinedData.push({
            ...
            month_year: `${item.month_name} ${item.year}`,
  ...
  useEffect(() => { applyFilters(); }, [data, filterValues]);
  useEffect(() => { updateChartData(selectedMonthsForChart); }, [selectedMonthsForChart, allMonthsData]);
  ...
  const handleNewRecord = (event) => {
    setData((prevData) => [event.detail, ...prevData]);
  };
  ...
  const toggleExpenseInChart = () => { setShowExpenseInChart(!showExpenseInChart); };
  const toggleActualInChart = () => { setShowActualInChart(!showActualInChart); };

  const columns = [
    { key: "month_date",    label: "Date",          classHead: "bg-dark text-white",      classBody: "bg-dark text-white" },
    { key: "day_of_week",   label: "Day",           classHead: "bg-secondary text-white", classBody: "bg-secondary text-white" },
    { key: "total_sales",   label: "Total Sales",   classHead: "bg-primary text-white",   classBody: "bg-primary text-white" },
    { key: "total_actual",  label: "Total Actual",  classHead: "bg-success text-white",   classBody: "bg-success text-white" },
    { key: "total_variance",label: "Variance",      classHead: "bg-warning text-dark",    classBody: "bg-warning text-dark" },
    { key: "total_expense", label: "Total Expense", classHead: "bg-danger text-white",    classBody: "bg-danger text-white" },
  ];
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-3 border rounded shadow">
          <p className="fw-bold mb-2">{`${label}${dataPoint ? ` (${dataPoint.month_year})` : ""}`}</p>
        ...
        <>
          <div className="card mb-3">
            <div className="card-header d-flex justify-content-between align-items-center">
              <div className="d-flex align-items-center">
                ...
                >
                  <span style={{ fontSize: "16px" }}>{isFilterPanelOpen ? "▼" : "▶"}</span>
                </button>
                <h5 className="mb-0">
                  Filters
              ...
              <div>
                <button className="btn btn-sm btn-outline-info me-2" onClick={toggleChart}>
                ...
                <div className="row row-cols-1 row-cols-md-3 row-cols-lg-4 g-2">
                  <div className="col">
                    <div className="form-floating">
                      <select className="form-select" id="monthSelect" value={selectedMonth} onChange={handleMonthChange}>
                  ...
                  <div className="col">
                    <div className="form-floating">
                      <select className="form-select" id="yearSelect" value={selectedYear} onChange={handleYearChange}>
                ...
                <div className="d-flex justify-content-between align-items-center mb-3">
                  ...
                  <div className="d-flex align-items-center gap-3">
                    <div className="form-check">
                      <input className="form-check-input" type="checkbox" id="showExpenseToggle7" checked={showExpenseInChart} onChange={toggleExpenseInChart} />
                      <label className="form-check-label" htmlFor="showExpenseToggle7">Show Total Expense</label>
                    </div>
                    <div className="form-check">
                      <input className="form-check-input" type="checkbox" id="showActualToggle7" checked={showActualInChart} onChange={toggleActualInChart} />
                      <label className="form-check-label" htmlFor="showActualToggle7">Show Total Actual</label>
                    </div>
                  </div>
                </div>
                <div className="row g-2 mb-3">
                  <div className="col-md-4">
                    <select onChange={(e) => handleAddMonthToChart(e.target.value)} value="">
                  ...
                  <div className="col-md-8">
                    <div className="d-flex gap-1 flex-wrap">
                      <span className="small text-muted me-2">Chart months:</span>
                      {selectedMonthsForChart.map((monthYear) => {
                        const [year, month] = monthYear.split("-");
                        return (
                          <span key={monthYear} className="badge bg-primary d-flex align-items-center gap-1">
                            ...
                            {selectedMonthsForChart.length > 1 && (
                              <button
                                ...
                                onClick={() => handleRemoveMonthFromChart(monthYear)}
                              ></button>
                            )}
                          </span>
                        );
                ...
                <div style={{ width: "100%", height: "400px" }}>
                  <ResponsiveContainer>
                    <LineChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 80 }}>
                      ...
                      <XAxis dataKey="full_date" angle={-45} textAnchor="end" height={80} interval="preserveStartEnd" tick={{ fontSize: 10 }}
                        tickFormatter={(value, index) => chartData.length > 31 ? (index % 5 === 0 ? value : "") : value}
                      />
                      <YAxis tickFormatter={(value) => `RM${value.toLocaleString()}`} />
                      <Tooltip content={<CustomTooltip />} />
                      <Legend />
                      <Line type="monotone" dataKey="total_sales" stroke="#0d6efd" strokeWidth={2} name="Total Sales" dot={{ fill: "#0d6efd", strokeWidth: 2, r: 4 }} activeDot={{ r: 6 }} />
                      {showActualInChart && (
                        <Line type="monotone" dataKey="total_actual" stroke="#198754" strokeWidth={2} name="Total Actual" dot={{ fill: "#198754", strokeWidth: 2, r: 4 }} activeDot={{ r: 6 }} />
                      )}
                      {showExpenseInChart && (
                        <Line type="monotone" dataKey="total_expense" stroke="#dc3545" strokeWidth={2} name="Total Expense" dot={{ fill: "#dc3545", strokeWidth: 2, r: 4 }} activeDot={{ r: 6 }} connectNulls={false} />
                      )}
                      <Brush dataKey="full_date" height={30} stroke="#8884d8" fill="#f0f0f0" />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </div>
          ...
          <div className="d-flex justify-content-between align-items-center mb-2">
            ...
            <div className="d-flex align-items-center">
              ...
              <select className="form-select form-select-sm" value={recordsPerPage} onChange={(e) => setRecordsPerPage(Number(e.target.value))} style={{ width: "auto" }}>
              ...
              <div className="table-responsive shadow rounded-3">
                <table className="table table-striped table-hover table-bordered mb-0">
                  <thead>
                    <tr>
                      {columns.map((column) => (
                        <th key={column.key} className={column.classHead}>{column.label}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {currentRecords.length > 0 ? (
                      currentRecords.map((record, index) => (
                        <tr key={`${record.month_date}-${index}`} style={{ cursor: "pointer" }}>
                          <td className={columns[0].classBody}>{record.month_date}</td>
                          <td className={columns[1].classBody}>{record.day_of_week}</td>
                          <td className={columns[2].classBody}>{formatCurrency(record.total_sales)}</td>
                          <td className={columns[3].classBody}>{formatCurrency(record.total_actual)}</td>
                          <td className={`${columns[4].classBody} ${getVarianceColor(record.total_variance)}`}>{formatCurrency(record.total_variance)}</td>
                          <td className={columns[5].classBody}>{record.total_expense !== null ? formatCurrency(record.total_expense) : "N/A"}</td>
                        </tr>
                      ))
                    ) : (
                      <tr>
                    ...
                    if (pageNumber === 1 || pageNumber === totalPages || (pageNumber >= currentPage - 1 && pageNumber <= currentPage + 1)) {
                      return (
                        <li key={pageNumber} className={`page-item ${currentPage === pageNumber ? "active" : ""}`}>
                          <button style={{ backgroundColor: "#E80000" }} className="page-link" onClick={() => paginate(pageNumber)}>{pageNumber}</button>
                        </li>
                      );
...
export default TableMonthSummary7;
