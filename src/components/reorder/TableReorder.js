...
const SOGO_API_URL = "http://121.121.232.54:88/mixue-sogo";
...
function TableReorder({ data, loading, apiBaseUrl, onChanged }) {
  ...
  const [alsoSogo, setAlsoSogo] = useState(false);
  ...
  const saveMin = (code, name) => {
    ...
    if (alsoSogo) {
      promises.push(
        fetch(`${SOGO_API_URL}/min_stock_update.php`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(body),
        }).then((res) => res.json())
      ...
      <Row className="mb-3 g-2">
        ...
        <Col md={5} className="d-flex align-items-center gap-3">
          ...
          <Form.Check
            type="checkbox"
            id="also-sogo"
            label="Also update Sogo DB"
            checked={alsoSogo}
            onChange={(e) => setAlsoSogo(e.target.checked)}
          />
        </Col>
