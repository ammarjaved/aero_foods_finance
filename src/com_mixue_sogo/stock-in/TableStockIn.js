...
function TableStockIn() {
  ...
  const handleSaveEdit = async (itemId) => {
    try {
      const response = await fetch(
        "http://121.121.232.54:88/mixue-sogo/update_stockin_trans.php",
        {
          method: "POST",
          headers: {
  ...
  const handleConfirmDelete = async (itemId, month_date) => {
    try {
      const response = await fetch(
        "http://121.121.232.54:88/mixue-sogo/del_stock.php",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    try {
      ...
      await Promise.all(
        selectedDateDetails.items.map((it) =>
          fetch("http://121.121.232.54:88/mixue-sogo/del_stock.php", {
            method: "POST",
