...
function ExpenseFormComponent() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "http://121.121.232.54:88/mixue-sogo/daily_expenditure.php",
        {
          method: "POST",
          headers: {
  ...
  const handleRowClick = async (record) => {
    ...
    if (record.expense_recipt) {
      ...
      if (record.expense_recipt.startsWith("data:image")) {
        ...
        const imageUrl = `http://121.121.232.54:88/mixue-sogo/${record.expense_recipt}`;
  ...
  const deleteRecord = async () => {
    ...
    try {
      const response = await fetch(
        "http://121.121.232.54:88/mixue-sogo/delete_daily_expenditure.php",
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
