<?php
// get_deductions.php - Fetch deductions for employees
...
try {
    ...
    if (isset($_GET['employee_name']) && !empty($_GET['employee_name'])) {
        $sql = "SELECT d.*, e.employment_type, e.basic_salary
                FROM employee_deductions d
                LEFT JOIN employees e ON LOWER(TRIM(e.short_name)) = LOWER(TRIM(d.employee_name))
                WHERE LOWER(TRIM(d.employee_name)) = LOWER(TRIM(:employee_name))
                ORDER BY d.deduction_type";
        ...
        $sql = "SELECT d.*, e.employment_type, e.basic_salary
                FROM employee_deductions d
                LEFT JOIN employees e ON LOWER(TRIM(e.short_name)) = LOWER(TRIM(d.employee_name))
                ORDER BY d.employee_name, d.deduction_type";
        $stmt = $conn->prepare($sql);
        $stmt->execute();
    }
    ...
    echo json_encode([
        'status' => 'success',
        'message' => "$count deduction(s) found",
        'results' => $results
    ]);
