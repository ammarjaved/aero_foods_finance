...
try {
    if ($isUpdate) {
        // UPDATE operation
		///$sql11="select nmae from log_sheet where start_time=";
    ...
    if ($isUpdate) {
		...
		$stmt->bindValue(':image_start_time', $data['image_start_time']);
		$stmt->bindValue(':image_end_time', $data['image_end_time']);
        $stmt->bindValue(':off_type', $data['off_type']);
    }else{
        //$stmt->bindValue(':month_date', $data['month_date']);
        //$stmt->bindValue(':day', $data['day']);
    }
